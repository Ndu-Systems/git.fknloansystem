...
if(isset($_GET['datepickerFrom'])){
    ...
    $datepickerTo = $_GET['datepickerTo'];
     $sql = "SELECT loan.*, customer.EmailAddress AS Email,customer.FirstName As Name,customer.LastName As Surname,customer.CellNumber As Number
             FROM loan
             INNER JOIN customer ON loan.CustomerId = customer.CustomerId 
             WHERE loan.Status = 0 AND loan.CreateDate >= '$datepickerFrom' and loan.CreateDate<= '$datepickerTo'";
...
$pdf->Cell(50,10,$row['Surname'],1,0,'C'); 
$pdf->Cell(45,10,$row['Number'],1,0,'C'); 
